...
import {
    ...
    SendJitsiJwtMessage,
    SendUserMessage, JoinRoomMessage
...
export class SocketManager {
    ...
    public handleJoinRoom(socket: UserSocket, joinRoomMessage: JoinRoomMessage): { room: GameRoom; user: User } {
        ...
        const {room, user} = this.joinRoom(socket, joinRoomMessage);
    ...
    private joinRoom(socket: UserSocket, joinRoomMessage: JoinRoomMessage): { room: GameRoom; user: User } {

        const roomId = joinRoomMessage.getRoomid();

        const world = this.rooms.get(roomId)
        if(world === undefined){
            throw new Error('Could not find room for ID: '+roomId)
        }
    ...
    private onRoomEnter(thing: Movable, listener: User) {
        if (thing instanceof User) {
            ...
            this.emitCreateUpdateGroupEvent(listener, thing);
        } else {
    ...
    private onClientLeave(thing: Movable, listener:User) {
        if (thing instanceof User) {
            this.emitUserLeftEvent(listener, thing.id);
        } else if (thing instanceof Group) {
            this.emitDeleteGroupEvent(listener, thing.getId());
        } else {
            console.error('Unexpected type for Movable.');
        }
    }

    private emitCreateUpdateGroupEvent(client: User, group: Group): void {
    ...
    private emitDeleteGroupEvent(user: User, groupId: number): void {
    ...
    private emitUserLeftEvent(user: User, userId: number): void {
