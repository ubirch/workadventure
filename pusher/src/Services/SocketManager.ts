...
import {
    ...
    SendJitsiJwtMessage,
    SendUserMessage, JoinRoomMessage, CharacterLayerMessage
...
export class SocketManager {
    ...
    async handleJoinRoom(client: ExSocketInterface): Promise<void> {
        ...
        try {
            ...
            apiClient.joinRoom(joinRoomMessage, (error: ServiceError|null, response: RoomJoinedMessage) => {
                if (error !== null) {
                    console.error('Error while calling joinRoom endpoint');
                    console.error(error.message);
                    return;
                }

                const serverToClientMessage = new ServerToClientMessage();
                serverToClientMessage.setRoomjoinedmessage(response);

                console.log('RoomJoinedMessage received!')
